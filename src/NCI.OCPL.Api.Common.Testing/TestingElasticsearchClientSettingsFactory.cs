...
public static class TestingElasticsearchClientSettingsFactory
{
  ...
  /// <returns>A new instance of <see cref="ElasticsearchClientSettings"/>.</returns>
  public static ElasticsearchClientSettings Create(IRequestInvoker connection)
  {
    // TODO: Check that this is a known type of in-memory connection, and throw an exception if not.
    // We want to avoid using a real connection in tests that are meant to be fully in-memory.

    // While this has a URI, it does not matter, we are explicitly using an
    // in-memory connection to avoid calling a real Elasticsearch instance.
