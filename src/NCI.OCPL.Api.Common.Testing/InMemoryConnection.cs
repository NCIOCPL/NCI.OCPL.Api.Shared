...
namespace NCI.OCPL.Api.Common.Testing
{
  ...
  /// </summary>
  /// <remarks>
  /// InMemoryRequestInvoker claims to implement IDisposable, but doesn't `Dispose(bool disposing)` overload;
  /// presumably because it doesn't actually make a database connection. Our class has nothing to dispose, and
  /// so explicitly relies on the base class to satisfy the IDisposable contract.
  /// </remarks>
